...
def main(args):
    ...
    if args.benchmark:
        logger.info('Validation with full-size images from DATA validation set')
        ...
        logger.info('Validation with randomly cropped images from DATA validation set')
...
def parser():
    ...
    parser.add_argument('-d', '--dataset', type=str, default='./DATA_BIN',
                        help='path to DATA dataset with images stored as numpy arrays')
    parser.add_argument('-s', '--scale', type=int, default=2, choices=[2],
    ...
    parser.add_argument('--benchmark', action='store_true',
                        help='validate with full-size DATA images after each epoch and save best models only')
