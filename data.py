...
class DATASequence(Sequence):
    ...
    def _hr_image_path(self, id):
        return os.path.join(self.path, f'DATA_{self.subset}_HR', f'{id:04}.npy')

    def _lr_image_path(self, id):
        return os.path.join(self.path, f'DATA_{self.subset}_LR_{self.downgrade}', f'X{self.scale}', f'{id:04}.npy')


def cropped_sequence(path, scale, subset, downgrade, image_ids=None, batch_size=16):
    return DATASequence(path=path, scale=scale, subset=subset, downgrade=downgrade, image_ids=image_ids,
